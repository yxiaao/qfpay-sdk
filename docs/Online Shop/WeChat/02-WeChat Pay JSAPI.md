...
Note: Merchants in Canada, please refer to [this](#alipay-online-payments) section for payment request and response parameters with `pay_type` 800207. <br/>
...
**Step 2:** Request the QFPAY order payment API `/trade/v1/payment` by providing the appointed `openid` and return the `pay_params` data, for further instructions please refer to the [API Endpoint for Payments](#api-endpoint-for-payments). 
