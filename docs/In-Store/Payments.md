...
:::note
  ...
  **(\*\*\*\*)**
  1. Please refer to [this](#alipay-online-payments) section for payment request and response.
:::
...
If you would like to quickly test the payment function in Postman we provide a collection that includes a pre-request script to generate the signature, download the file from here: [Payment Request in Postman](https://sdk.qfapi.com/files/qfpay_openapi_payment_request.postman_collection.json)
